...
export default function EditCategoryPage() {
  ...
  const { data: categoryData, isLoading: isCategoryLoading, error: categoryError } = useQuery({
    queryKey: ['admin-category-detail', categoryId],
    queryFn: () => categoriesService.findOne(categoryId),
    enabled: !!categoryId
  });

  // Fetch flat categories list for Parent Category selection
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: any) => categoriesService.update(categoryId, payload),
    onSuccess: () => {
    ...
    return (
      <DashboardLayout>
        <div className="max-w-5xl mx-auto text-center py-12">
          ...
          <h2 className="text-lg font-bold text-slate-800">Failed to Load Category</h2>
          <p className="text-sm text-slate-500 mt-2">The category might not exist or there was a system retrieval error.</p>
          <Button onClick={() => router.push('/categories')} className="mt-6">
            Back to Categories
          </Button>
        </div>
      </DashboardLayout>
      ...
      <div className="space-y-6 max-w-5xl mx-auto pb-12">
        ...
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h1 className="text-xl font-bold text-slate-800 leading-tight">Edit Category</h1>
                 <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Taxonomy Management / Edit Category</p>
              </div>
           </div>
        ...
        {/* Form Body */}
        <form id="edit-category-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-1">
                       ...
                       >
                         ...
                         {categoriesData?.items
                           ?.filter((cat: any) => cat.id !== categoryId) // Prevent setting self as parent
                           ?.map((cat: any) => (
