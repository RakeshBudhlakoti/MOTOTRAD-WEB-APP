...
export default function EditBucketPage() {
  ...
  const { data: basket, isLoading: isFetching } = useQuery({
    queryKey: ['admin-basket', id],
    queryFn: () => basketsService.findOne(id),
    enabled: !!id,
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => basketsService.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-baskets'] });
      queryClient.invalidateQueries({ queryKey: ['admin-basket', id] });
        ...
        <div className="flex items-center justify-between border-b border-slate-200 pb-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-lg font-black uppercase tracking-wider text-slate-800">Edit Bucket</h1>
                <p className="text-[10px] text-slate-400 font-medium">Modify this collection event or location group.</p>
              </div>
            </div>
          </div>
        </div>
